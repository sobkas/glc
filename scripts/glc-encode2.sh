...
ls -lh $tmp
echo $bitrate
glc-play $2 -o - -y 1|avconv -i - -y -threads auto -b:v "$bitrate"k -r 25 $tmp/video.mkv 
...
avconv -y -i $tmp/video.mkv  -i $tmp/audio-out.wav -vcodec copy -b:v 170k -b:a 128k $2.mkv
rm -r $tmp
#rm $2
fi
